...
import { getMPs } from "@/lib/data/mps";
...
export default async function MPsPage({ params: { locale } }: { params: { locale: string } }) {
  const t = await getTranslations({ locale, namespace: "mps" });
  const mps = await getMPs({ status: "active", isCurrentMember: true });
  ...
  return (
    <div className="page-container py-8">
      ...
      {/* MPs by party */}
      {parties.map((party) => (
        <section key={party} className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <PartyBadge party={party} partyCode={getPartyCode(party)} size="lg" />
            <span className="text-sm text-ink-500">({byParty[party].length} MPs)</span>
          </div>
                        ...
                        {mp.stats && (
                          <div className="mt-2 space-y-1 text-xs text-ink-500">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono">{mp.stats.totalVotes}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>{t("card.attendance")}:</span>
                              <span className="font-mono">{Math.round(mp.stats.attendance)}%</span>
                            </div>
                            <div className="flex justify-between">
                              <span>{t("card.partyLoyalty")}:</span>
                              <span className="font-mono">{Math.round(mp.stats.partyAlignmentRate)}%</span>
                            </div>
                          </div>
                        )}
